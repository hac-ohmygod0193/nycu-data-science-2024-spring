...
def create_feature_extractor(name, list_features, cuda=True, **kwargs):
    ...
    vprint(
        verbose, f'Creating feature extractor "{name}" with features {list_features}'
    )
    ...
    if cuda:
        feat_extractor.cuda()
    if (
        get_kwarg("feature_extractor_compile", kwargs)
        and feat_extractor.can_be_compiled()
    ):
        feat_extractor = torch_maybe_compile(
            feat_extractor,
            feat_extractor.get_dummy_input_for_compile().to(
    ...
    if out is None:
        for metric in ("isc", "fid", "kid", "prc"):
            if get_kwarg(metric, kwargs):
                ...
                vassert(
                    out in (None, default_fe),
                    "Cannot have several feature extractors in one call",
                )
                out = default_fe
        vassert(out is not None, f"Feature extractor was not resolved")
    vassert(
        out in FEATURE_EXTRACTORS_REGISTRY, f'Feature extractor "{out}" not registered'
    )
    return out
...
def resolve_feature_layer_for_metric(metric, **kwargs):
    ...
    if out is None:
        ...
        vassert(
            name_fe in FEATURE_EXTRACTORS_REGISTRY,
            f'Feature extractor "{name_fe}" not registered',
        )
