...
def calculate_precision_recall_part(
    features_1, features_2, neighborhood=3, batch_size=10000
):
    # Precision
    dist_nn_1 = []
    for feat_1_batch in features_1.split(batch_size):
        dist_nn_1.append(
            calc_cdist_part(feat_1_batch, features_1, batch_size)
    ...
    for feat_2_batch in features_2.split(batch_size):
        dist_nn_2.append(
            calc_cdist_part(feat_2_batch, features_2, batch_size)
            .kthvalue(neighborhood + 1)
            .values
        )
...
def prc_features_to_metric(features_1, features_2, **kwargs):
    ...
    calculate_precision_recall_fn = (
        calculate_precision_recall_part
        if save_cpu_ram
        else calculate_precision_recall_full
    )
    precision, recall = calculate_precision_recall_fn(
...
def prc_featuresdict_to_metric(
...
def calculate_prc(**kwargs):
    ...
    metric = prc_featuresdict_to_metric(
        featuresdict_1, featuresdict_2, feat_layer_name, **kwargs
    )
    return metric
