...
def kernel_rbf(X, Y, **kwargs):
    ...
    K = np.exp(
        (
            2 * XY
            - np.outer(XX, np.ones(YY.shape[0]))
            - np.outer(np.ones(XX.shape[0]), YY)
        )
        / (2 * sigma**2)
    )
    return K
...
def kid_features_to_metric(features_1, features_2, **kwargs):
    ...
    vprint(
        verbose,
        f"Kernel Inception Distance: {out[KEY_METRIC_KID_MEAN]:.7g} ± {out[KEY_METRIC_KID_STD]:.7g}",
    )
...
def kid_featuresdict_to_metric(
    ...
    if metric[KEY_METRIC_KID_MEAN] < 0 and get_kwarg("verbose", kwargs):
        print(
            "KID values slightly less than 0 are valid and indicate that distributions are very similar"
        )
    return metric
