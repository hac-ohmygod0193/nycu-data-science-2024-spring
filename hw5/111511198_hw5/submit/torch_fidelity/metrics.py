...
def calculate_metrics_one_feature_extractor(**kwargs):
    ...
    vassert(
        ...
        "Second input is required for binary metrics",
    )
    ...
    if have_other_than_ppl:
        ...
        if input5 is not None:
            vprint(verbose, "Extracting features from input5")
            featuresdict_5 = extract_featuresdict_from_input_id_cached(5, feat_extractor, **kwargs)

            for key, val in featuresdict_5.items():
                featuresdict_1[key] += val
