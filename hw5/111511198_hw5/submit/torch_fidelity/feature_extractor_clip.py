...
MODEL_URLS = {
    k: f'{MODEL_BASE_URL}/{v["hash"]}/{v["filename"]}.pt'
...
class FeatureExtractorCLIP(FeatureExtractorBase):
    def __init__(
        ...
        **kwargs,
        ...
        vassert(
            name in MODEL_URLS,
            f"Model {name} not found; available models = {list(MODEL_URLS.keys())}",
        )
        vassert(
        ...
        if feature_extractor_weights_path is None:
            ...
            model_jit = torch.jit.load(
                feature_extractor_weights_path, map_location="cpu"
            )

        self.model = build_model(
            model_jit.state_dict(), self.feature_extractor_internal_dtype
        )
