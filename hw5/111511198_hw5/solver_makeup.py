...
class Solver_makeupGAN(object):
    def __init__(self, data_loaders, config, dataset_config):
        ...
        # Training settings
        self.snapshot_epoch = config.snapshot_epoch
    ...
    def train(self):
        ...
        for self.e in tqdm(range(start, self.num_epochs)):
            ...
            for self.i, (img_A, img_B, mask_A, mask_B) in enumerate(tqdm(self.data_loader_train)):
                ...
                # mask attribute: 0:background 1:right_eye 2:right_eyebrow 3:left_ear 4:face 5: right_ear 6: left_eye 
                # 7: left_eyebrow 8:nose 9:upper_lip 10:neck 11:teeth 12: hair 13: under_lip 
            ...
            self.writer.add_scalar('training_losses/GA-loss-adv', g_A_loss_adv.item(), self.e)
            self.writer.add_scalar('training_losses/GB-loss-adv', g_B_loss_adv.item(), self.e)
            self.writer.add_scalar('training_losses/rec-org', g_loss_rec_A.item(), self.e)
            self.writer.add_scalar('training_losses/rec-ref', g_loss_rec_B.item(), self.e)
            self.writer.add_scalar('training_losses/vgg-A', g_loss_A_vgg.item(), self.e)
            self.writer.add_scalar('training_losses/vgg-B', g_loss_B_vgg.item(), self.e) 
            # Save model checkpoints every snapshot epoch
            if (self.e + 1) % self.snapshot_epoch == 0:
                self.save_models()
            ...
            torch.save(self.G.state_dict(),
                   os.path.join(self.snapshot_path, 'resume_checkpoint_G.pth'))
            for i in self.cls:
                torch.save(getattr(self, "D_" + i).state_dict(),
                        os.path.join(self.snapshot_path, 'resume_checkpoint_D_' + i + '.pth'))
