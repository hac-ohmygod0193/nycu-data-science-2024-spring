...
from modeling import EMA, UNet, AttentionUNet
...
def create_str_loss(loss: Dict[str, torch.Tensor], use_ssl: bool = True) -> str:
    ...
    for key, value in loss.items():
        loss_list.append(f"{key}: {value:.5f}")
...
class Trainer:
    def __init__(self, cfg):
        ...
        if cfg.TRAIN.MODEL == "AttetionUNet":
            model = AttentionUNet(img_ch=cfg.MODEL.IN_CHANNELS, output_ch=cfg.DATA.NUM_CLASSES)
        else:
        ...
        if cfg.TRAIN.USE_SSL:
            ...
            if cfg.TRAIN.MODEL == "AttentionUNet":
                ema_model = AttentionUNet(
